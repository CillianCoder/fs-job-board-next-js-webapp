...
import {
  Search, FileText, User, Mail, Phone, Link2, Code2, Calendar,
  X, Loader2, ChevronLeft, ChevronRight, Briefcase, Info,
  AlertCircle, Building2, CheckCircle2, MessageSquare, ClipboardList,
  ExternalLink
} from "lucide-react";
import ApplicationStatusBadge from "@/components/applications/ApplicationStatusBadge";
import { updateApplicationStatusAdminAction } from "@/app/actions/admin";
...
export default function ApplicationsTable({
  ...
  const applyFilters = (queryStr = searchVal, statusStr = statusFilter, limitStr = pageSize) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex flex-col md:flex-row gap-4 items-center justify-between">
        <h2 className="text-xl font-bold text-foreground self-start md:self-center">Applications Log</h2>
          ...
          <tbody className="divide-y divide-gray-150 dark:divide-gray-800 text-sm">
            {applications.length === 0 ? (
              ...
              applications.map((app) => (
                <tr key={app.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/25 transition-colors">
                  ...
                  <td className="px-6 py-4 font-semibold text-foreground">{app.job.title}</td>
      ...
      {totalPages > 1 && (
        <div className="p-6 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between">
          <span className="text-sm text-foreground/50 font-medium">
            ...
            <span className="font-semibold text-foreground">{totalPages}</span> ({totalApplications} total applications)
          </span>
          ...
          <div className="relative z-10 w-full max-w-3xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl p-6 overflow-hidden animate-in fade-in zoom-in-95 duration-150 max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-start justify-between pb-4 border-b border-gray-150 dark:border-gray-850 shrink-0">
              <div>
                ...
                <p className="text-xs text-foreground/50 mt-0.5">Applied for <span className="font-semibold text-foreground/75">{selectedApplication.job.title}</span> at <span className="font-semibold text-foreground/75">{selectedApplication.job.company}</span></p>
              </div>
              <button onClick={() => setSelectedApplication(null)} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-foreground/50 hover:text-foreground transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="p-3 bg-gray-50 dark:bg-gray-950 border border-gray-250 dark:border-gray-850 rounded-xl">
                      <span className="text-[10px] font-bold text-foreground/40 uppercase tracking-wider block">Candidate Name</span>
                      <p className="text-sm font-semibold text-foreground mt-0.5">{selectedApplication.name}</p>
                    </div>
                    <div className="p-3 bg-gray-50 dark:bg-gray-950 border border-gray-250 dark:border-gray-850 rounded-xl">
                      <span className="text-[10px] font-bold text-foreground/40 uppercase tracking-wider block">Email Address</span>
                      <p className="text-sm font-semibold text-foreground mt-0.5 font-mono truncate">{selectedApplication.email}</p>
                    </div>
                    <div className="p-3 bg-gray-50 dark:bg-gray-950 border border-gray-250 dark:border-gray-850 rounded-xl">
                      <span className="text-[10px] font-bold text-foreground/40 uppercase tracking-wider block">Phone</span>
                      <p className="text-sm font-semibold text-foreground mt-0.5">{selectedApplication.phone || "Not provided"}</p>
                    </div>
                    <div className="p-3 bg-gray-50 dark:bg-gray-950 border border-gray-250 dark:border-gray-850 rounded-xl">
                      <span className="text-[10px] font-bold text-foreground/40 uppercase tracking-wider block">Experience</span>
                      <p className="text-sm font-semibold text-foreground mt-0.5">{getExperienceLabel(selectedApplication.experience)}</p>
                    </div>
                  </div>
                  ...
                  {/* Cover Letter */}
                  <div>
                    <h4 className="text-xs font-bold text-foreground/40 uppercase tracking-wider mb-2">Cover Letter Text</h4>
                    <div className="p-3.5 bg-gray-50 dark:bg-gray-950 border border-gray-250 dark:border-gray-850 rounded-xl text-xs leading-relaxed text-foreground/80 whitespace-pre-wrap max-h-48 overflow-y-auto">
                      {selectedApplication.coverLetter || "No cover letter submitted for this job."}
                    </div>
                  </div>
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-850 rounded-2xl flex flex-col gap-4">
                  ...
                  <form onSubmit={handleStatusSubmit} className="space-y-4">
                    ...
                    {/* Status Dropdown */}
                    <div>
                      <label className="block text-xs font-bold text-foreground/60 uppercase tracking-wider mb-1.5">Application Status</label>
                      <select
                        value={editStatus}
                    ...
                    {/* Notes Textarea */}
                    <div>
                      <label className="block text-xs font-bold text-foreground/60 uppercase tracking-wider mb-1.5">Recruiter / Admin Notes</label>
                      <textarea
                        rows={5}
